...
import os
import time
from PanGan_RGB import PanGan
from DataSet_RGB import DataSet
from config_RGB import FLAGES

def print_current_training_stats(error_pan_model, error_uav_model, error_ms_model, error_g_model, global_step, learning_rate, time_elapsed):
    ...
    losses += ' | spectrual loss (UAV): {}'.format(error_uav_model)
...
def main(argv):
    model=PanGan(FLAGES.pan_size, FLAGES.uav_size, FLAGES.ms_size, FLAGES.batch_size, FLAGES.num_spectrum, FLAGES.ratio,
                 FLAGES.lr,FLAGES.decay_rate,FLAGES.decay_step,is_training=True)
    model.train()
    dataset=DataSet(FLAGES.pan_size, FLAGES.uav_size, FLAGES.ms_size, FLAGES.img_path, FLAGES.data_path, FLAGES.batch_size,
                    FLAGES.stride)
    DataGenerator=dataset.data_generator
    print(DataGenerator)
    
    dataset_valid=DataSet(FLAGES.pan_size, FLAGES.uav_size, FLAGES.ms_size, FLAGES.img_path, FLAGES.data_path, FLAGES.batch_size,
                    FLAGES.stride, 'valid')
    ...
    with tf.Session() as sess:
        ...
        for training_itr in range(FLAGES.iters):
            t1 = time.time()
            pan_batch, uav_batch, ms_batch=next(DataGenerator)
            
            for i in range(2):
                _, error_pan_model = sess.run([model.train_spatial_discrim, model.spatial_loss],feed_dict={model.pan_img: pan_batch, model.uav_img: uav_batch, model.ms_img: ms_batch})
                _, error_ms_model = sess.run([model.train_spectrum_discrim, model.spectrum_loss],feed_dict={model.pan_img: pan_batch, model.uav_img: uav_batch, model.ms_img: ms_batch})
                _, error_uav_model = sess.run([model.train_spectrum_discrim_uav, model.spectrum_loss_uav],feed_dict={model.pan_img: pan_batch, model.uav_img: uav_batch, model.ms_img: ms_batch})
            ...
            _, error_g_model, global_step, summary, learning_rate = sess.run([model.train_Pan_model, model.g_loss,model.global_step, merge_summary,model.learning_rate],
                                                                             feed_dict={model.pan_img: pan_batch, model.ms_img: ms_batch, model.uav_img: uav_batch})
            error_pan_model=sess.run(model.spatial_loss,feed_dict={model.pan_img: pan_batch, model.uav_img: uav_batch, model.ms_img: ms_batch})
            
            print(model.spatial_loss)
            print("-------------- error_pan_model: ", error_pan_model)
            
            error_ms_model=sess.run(model.spectrum_loss,feed_dict={model.pan_img: pan_batch, model.uav_img: uav_batch, model.ms_img: ms_batch})	
            print("-------------- error_ms_model: ", error_ms_model)
            
            				
            print_current_training_stats(error_pan_model, error_ms_model, error_uav_model, error_g_model, global_step, learning_rate, time.time()-t1)
