...
class DataSet(object):
    ...
    def generator(self):
        ...
        while True:
            ...
            batch_ms=np.zeros((self.batch_size,self.ms_size,self.ms_size,8))
    ...
    def all_data(self, root_ms, root_pan):
        ...
        for i, path in enumerate(paths_pan):
            ...
            pkl_data = pkl_data1.reshape(pkl_data1.shape[0], pkl_data1.shape[1], 1)
            print(path, pkl_data.shape)
            all_pan.append(pkl_data)
    
        print(len(all_lrms), len(all_pan))
    
        allData = [all_lrms, all_pan]
        return allData    
    ...
    def make_data(self, source_path, data_save_path, stride):
        ...
        source_ms_path=os.path.join(source_path, 'MS_inputs')
        print(source_ms_path)
        source_pan_path=os.path.join(source_path, 'Pan_inputs')
        

        #all_pan=self.crop_to_patch(source_pan_path, stride, name='pan')
        #all_ms=self.crop_to_patch(source_ms_path, stride, name='ms')
        all_in_data = self.all_data(source_ms_path, source_pan_path)

        all_ms = all_in_data[0]
        all_pan = all_in_data[1]
