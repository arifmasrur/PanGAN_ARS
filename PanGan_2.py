...
class PanGan(object):
    ...
    def build_model(self, batch_size, num_spectrum, is_training):
        ...
        if is_training:
            ...
            ms_img=tf.image.resize_images(self.ms_img, [207, 181], method=1)
            self.ms_img_int = tf.image.resize_images(self.ms_img, [207, 181], method=1)
        ...
        with tf.variable_scope('Pan_model'):
            if self.is_training:
                with tf.name_scope('upscale'):
                    ...
                    ms_img=tf.image.resize_images(ms_img, [207, 181], method=2)
            
            #print("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^:", ms_img)
            #print("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^:", pan_img) 
            
            #pan_img=tf.image.resize_images(pan_img, [5, 5], method=2)
            ms_img=tf.image.resize_images(ms_img, [207, 181], method=1)
    ...
    def high_pass_1(self, img, type='PanSharepening'):
        if type=='pan':
            ...
            #img = tf.image.resize_images(img, [207, 181], method=2)
            input=img
            for i in range(7):
